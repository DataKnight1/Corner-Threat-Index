...
$$\text{CTI} = (y_1 \times y_2) - 0.1(y_3 \times y_4) + 5.0(y_5)$$

**Component Interpretation**:
1. **$(y_1 \times y_2)$**: Probability of dangerous shot (shot AND from high-threat location)
2. **$-0.1(y_3 \times y_4)$**: Penalty for defensive disruption (clearance/turnover)
3. **$+5.0(y_5)$**: Reward for territorial gain (ball progression)

**Coefficient Rationale**:
- $\lambda = 0.1$: Reduced penalty to avoid over-penalizing fruitful but risky attacking teams (e.g., Arsenal)
- $\gamma = 5.0$: Increased reward for territorial gain to value territory advancement even if no immediate shot occurs
